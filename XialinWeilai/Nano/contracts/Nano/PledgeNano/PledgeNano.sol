...
/*
 * 合约：PledgeNano
 * 规则：通过质押nano产生nt锁仓在合约内，且每次只能质押1张
 *      父子绑定，子可为父加速，被加速的父可立即释放fixSpeedAmount个nano
*/
contract PledgeNano is Ownable, ReentrancyGuard {
    ...
    uint256 private constant lockDurationTime = 25 minutes;
    uint256 public constant lockDuration = 10;
    uint256 public constant releaseNtTokenPerDay = 33e16;
    // uint256 public constant releaseInterval = 1 minutes;
    uint256 public constant releaseInterval = 30 seconds;

    uint256 constant fixSpeedAmount = 10 * 10**18;

    mapping(address => mapping(uint256=>LockInfo)) private stakes;
    mapping(address => mapping(uint256 => uint256)) userStakeIds;
    mapping(address => address[]) parentToChilds;
    ...
    mapping(address => uint256[]) accountHasStakeIdAmounts;
    mapping(address => mapping(address=>bool)) public pchasbindornot;
    mapping(address => ClaimInfo[]) userClaimInfo;
    ...
    function NanoStaking(uint256 _amount) external nonReentrant {
        if(_amount != 1) {
            revert ParameterError(_amount,"You can only stake 1 Nano token!");
        }
        ...
        userStakeIds[msg.sender][_amount]++;
        uint256 stakeId = userStakeIds[msg.sender][_amount];
    ...
    function speedForParent() private {
        ...
        if (hasParent[msg.sender] == true && isStaked[_parent] == true) {
            ...
            for (uint256 i = 0; i < parentStakeIds.length; i++) {
                ...
                // 如果 norelease 大于0,加速父级
                if (norelease > 0) {
                    if (norelease >= fixSpeedAmount) {
                        parentLock.hasreleaseall = parentLock.norelease - fixSpeedAmount;
    ...
    function accumulateRealease2(address _player, uint256 _stakeId) public view returns(uint256) {
        ...
        // 如果在锁仓期限内，且未释放量大于等于10
        if (daysPassed < lockDuration && userLockInfo.norelease >= fixSpeedAmount) {
            // 如果没有子给父加速，就不会执行累积加上立即释放10个的操作 userLockInfo.speedCount * fixSpeedAmount
            // 如果有子给父加速，就会执行累积加上立即释放10个的操作  userLockInfo.speedCount * fixSpeedAmount
            accumulatedRelease = daysPassed * releaseNtTokenPerDay + userLockInfo.speedCount * fixSpeedAmount;
        }

        // 如果在锁仓期限内，且未释放量小于10，且没有子给父加速,就正常每天释放0.33个
        if(daysPassed < lockDuration && userLockInfo.norelease < fixSpeedAmount && isSpeedCountMarked[_player][_stakeId][userLockInfo.speedCount] == false) {
            accumulatedRelease = daysPassed * releaseNtTokenPerDay;
        }

        // 如果在锁仓期限内，且未释放量小于10，且有子给父加速,就立即释放，也就是 accumulatedRelease 为 100
    ...
    function getAllStakeidsAmounts(address _address) public view returns(uint256) {
        return accountHasStakeIdAmounts[_address].length;
    }
